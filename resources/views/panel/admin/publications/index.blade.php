    ...
    <div class="page-body pt-6">
        <div class="container-xl">
            <div class="d-flex mb-3">
                ...
                <a onclick="return toastr.info('This feature is disabled in Demo version.')" class="btn btn-primary mx-2">{{__('Create New Publication')}}</a>
                @else
                    <a href="{{route('dashboard.admin.publications.create')}}" class="btn btn-primary mx-2">{{__('Create New Publication')}}</a>

                @endif
            </div>
            <div class="mb-3">
                <form action="{{ route('dashboard.admin.publications.index') }}" method="GET">
            ...
            <div class="card">
				...
				<div id="table-default" class="card-table table-responsive">
					...
					<table class="table">
						...
						@foreach($publicationList as $entry)
							<tr>
								<td id="{{$entry->slug}}" class="sort-group">{{$entry->title}}</td>
								...
								<td class="whitespace-nowrap">
                                        ...
                                        <a  href="{{route('dashboard.admin.publications.updateStatus', $entry->id)}}#{{$entry->slug}}" class="btn w-[36px] h-[36px] p-0 border hover:bg-red-500 hover:text-white">
                                           @if($entry->status)
                                           <svg height="16" width="16"><circle cx="8" cy="8" r="8" fill="#00FF00"/></svg>
                                           @else
                                           <svg height="16" width="16"><circle cx="8" cy="8" r="8" fill="#FF0000"/></svg>
                                           @endif
                                        </a>                                       <a href="{{route('dashboard.admin.publications.create', $entry->id)}}"
                                        ...
                                        <a onclick="return confirm(`{{__('Do you want to delete this publication? This is irreversible.')}}`);" href="{{route('dashboard.admin.publications.delete', $entry->id)}}" class="btn w-[36px] h-[36px] p-0 border hover:bg-red-500 hover:text-white" title="{{__('Delete')}}">
                    ...
                    <div class="mt-4">
                        {{ $publicationList->appends(request()->query())->links() }}
                    </div>
				</div>
            </div>
        </div>
