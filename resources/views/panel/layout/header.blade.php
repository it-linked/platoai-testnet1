        ...
        <div class="p-0 container-fluid max-lg:w-full">
            <h1 class="flex items-center ps-[1.25rem] pe-9 min-h-[--lqd-header-height] max-w-full relative max-lg:hidden group-[.navbar-shrinked]/body:w-full group-[.navbar-shrinked]/body:px-0 group-[.navbar-shrinked]/body:text-center group-[.navbar-shrinked]/body:justify-center">
                <a class="block px-0" href="{{ LaravelLocalization::localizeUrl( route('dashboard.index') ) }}">
                    ...
                    <img src="/{{$setting->logo_dashboard_dark_path}}" @if ( isset($setting->logo_dashboard_dark_2x_path) )
                    ...
                    <img src="/{{$setting->logo_collapsed_dark_path}}" @if ( isset($setting->logo_collapsed_dark_2x_path) )
            ...
            <div class="collapse navbar-collapse" id="navbar-menu">
                <ul class="primary-nav navbar-nav max-lg:py-4 max-lg:px-3">
                    ...
                    @endif
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{ activeRoute(['dashboard.user.openai.list', 'dashboard.user.openai.generator', 'dashboard.user.openai.articlewizard.new', 'dashboard.user.openai.generator.workbook', 'dashboard.user.puretalk.speech.show', 'dashboard.user.openai.generator', 'dashboard.user.openai.generator', 'ai_speech_to_text', 'dashboard.user.openai.generator', 'ai_voiceover']) }}" href="#navbar-help" data-bs-toggle="dropdown" data-bs-auto-close="false" role="button" aria-expanded="false">
                        ...
                        <div class="dropdown-menu {{ activeRouteBulkShow(['dashboard.user.openai.list', 'dashboard.user.openai.generator', 'dashboard.user.openai.articlewizard.new', 'dashboard.user.openai.generator.workbook', 'dashboard.user.puretalk.speech.show', 'dashboard.user.openai.generator'], 'show') }}">
                        @if ( $setting->feature_ai_writer )
                            ...
                            <a class="nav-link {{ route('dashboard.user.puretalk.speech.show') == url()->current() ? 'active' : '' }}" href="{{route('dashboard.user.puretalk.speech.show')}}">
                                {{__('PureTalk AI')}} <span style="font-size: 9px;margin-left: 3px;margin-top: -10px;">TM</span>
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{activeRoute('dashboard.user.vertical.name')}}" href="#navbar-help" data-bs-toggle="dropdown" data-bs-auto-close="false" role="button" aria-expanded="false">
                            <span class="nav-link-icon" style="stroke:unset">
                                <svg style="position:relative;left:2px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" id="Category">
                                    <path fill="#6c7a91" d="M12 2l-5.5 9h11L12 2zm0 3.84L13.93 9h-3.87L12 5.84zM17.5 13c-2.49 0-4.5 2.01-4.5 4.5s2.01 4.5 4.5 4.5 4.5-2.01 4.5-4.5-2.01-4.5-4.5-4.5zm0 7c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5zM3 21.5h8v-8H3v8zm2-6h4v4H5v-4z">
                                    </path>
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{activeRoute('dashboard.publication.externalSite')}}" href="#navbar-help" data-bs-toggle="dropdown" data-bs-auto-close="false" role="button" aria-expanded="false">
                            <span class="nav-link-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="svg-icon" viewBox="0 0 24 24" fill="none">
                                    <path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 9H9V9h10v2zm-4 4H9v-2h6v2zm4-8H9V5h10v2z"></path>
                                </svg>
                        ...
                        <div class="dropdown-menu {{ activeRouteBulkShow(['dashboard.publication.externalSite']) }}">
                            ...
                            <span id="{{$publication->slug}}"></span>
                            <a class="nav-link {{ request()->url() == route('dashboard.publication.externalSite', ['slug' => $publication->slug]) ? 'active' : '' }}" href="{{ route('dashboard.publication.externalSite', ['slug' => $publication->slug]) }}#{{$publication->slug}}">
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu {{ activeRouteBulkShow( ['dashboard.vertical.list', 'dashboard.vertical.category.list'] ) }}">
                            <a class="dropdown-item {{activeRoute('dashboard.vertical.list')}}" href="{{route('dashboard.vertical.list')}}">
                                {{__('Listing')}}
                            </a>
                            <a class="dropdown-item {{activeRoute('dashboard.vertical.category.list')}}" href="{{route('dashboard.vertical.category.list')}}">
                                {{__('Category')}}
                            </a>
                            <a class="dropdown-item {{activeRoute('dashboard.vertical.crawl.list')}}" href="{{route('dashboard.vertical.crawl.list')}}">
                                {{__('Crawl')}}
                            </a>
                        </div>
                    </li>
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle {{ activeRouteBulk( ['dashboard.admin.sitemap.index', 'dashboard.admin.sitemap.links.index'] ) }}" href="#navbar-help" data-bs-toggle="dropdown" data-bs-auto-close="false" role="button" aria-expanded="false">
                            <span class="nav-link-icon">
                        ...
                        <div class="dropdown-menu {{ activeRouteBulkShow( ['dashboard.admin.sitemap.index', 'dashboard.admin.sitemap.links.index'] ) }}">
                            <a class="dropdown-item {{ activeRoute('dashboard.admin.sitemap.index')}}" href="{{route('dashboard.admin.sitemap.index')}}">
                                {{__('General')}}
                            </a>
                            <a class="dropdown-item {{activeRoute('dashboard.admin.sitemap.links.index')}}" href="{{route('dashboard.admin.sitemap.links.index')}}">
                                {{__('Add Links')}}
                            </a>
                        </div>
                    </li>
                        ...
                        <div class="nav-link-label transition-all">
                            <div class="inline-block text-center text-[13px] leading-none text-muted border-solid border-[var(--tblr-border-color)] rounded-[var(--tblr-border-radius)] p-[1rem_2rem]">
                                ...
                                <p class="mb-[0.75em]">{{__('Invite your friend and get')}}
                                    {{$setting->affiliate_commission_percentage}}% {{__('on all their purchases.')}}
                                </p>
                                <p class="m-0">
            ...
            <div class="flex gap-[18px] max-lg:gap-2">
                <div class="flex items-center max-xl:gap-2 max-lg:hidden xl:gap-3">
                    ...
                    <a class="btn" href="{{route('dashboard.admin.index')}}">
                        <svg class="hidden max-lg:block" xmlns="http://www.w3.org/2000/svg" height="20" fill="none" viewBox="0 96 960 960" width="20">
                    ...
                    <a class="btn btn-primary" href="{{route('dashboard.user.payment.subscription')}}">
                        <svg class="md:me-2 max-lg:w-[20px] max-lg:h-[20px]" width="11" height="15" viewBox="0 0 11 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="flex items-center">
                    <a href="{{ theme_url('dark') }}" class="nav-link items-center justify-center px-0 hide-theme-dark hover:!bg-transparent max-lg:w-10 max-lg:h-10 max-lg:p-0 max-lg:border max-lg:border-solid max-lg:border-[--tblr-border-color] max-lg:!rounded-full max-lg:dark:bg-white max-lg:dark:bg-opacity-[0.03]" title="{{__('Enable dark mode')}}">
        ...
        <div class="flex items-center lg:-order-1 max-lg:w-full max-lg:fixed max-lg:bottom-16 max-lg:left-0 max-lg:z-50">
            <form class="navbar-search group !me-2 max-lg:hidden max-lg:[&.show]:flex max-lg:[&.collapsing]:flex max-lg:m-0 max-lg:w-full max-lg:!me-0" id="navbar-search" autocomplete="off" novalidate>
                <div class="w-full input-icon max-lg:p-3 max-lg:bg-[#fff] max-lg:dark:bg-zinc-800">
                    ...
                    <input type="search" class="form-control navbar-search-input peer max-lg:!rounded-md dark:!bg-zinc-900" id="top_search_word" onkeydown="return event.key != 'Enter';" placeholder="{{__('Plato search directory')}}" aria-label="Search in website">
                    ...
                    <span class="absolute !end-3 top-1/2 -translate-y-1/2 -translate-x-2 transition-all opacity-0 pointer-events-none peer-focus:!opacity-100 peer-focus:translate-x-0  group-[.is-searching]:hidden group-[.done-searching]:hidden rtl:-scale-x-100">
                        <svg xmlns="http://www.w3.org/2000/svg" height="25" viewBox="0 96 960 960" width="25" fill="none">
                    ...
                    <div class="navbar-search-results absolute top-[calc(100%+17px)] !start-0 bg-[#fff] shadow-[0_10px_70px_rgba(0,0,0,0.1)] rounded-md w-[100%] max-h-[800px] overflow-y-auto hidden group-[.done-searching]:block dark:!bg-[--tblr-bg-surface] max-lg:top-auto max-lg:bottom-full max-lg:start-0 max-lg:end-0 max-lg:w-auto" id="search_results" style="z-index: 999;">
                        <!-- Search results here -->
                        <h3 class="m-0 py-[0.75rem] px-3 border-solid border-b border-t-0 border-r-0 border-l-0 border-[--tblr-border-color] text-[1rem] font-medium">
                            {{__('Search results')}}
                        </h3>
    ...
    <ul class="grid grid-cols-4 place-items-center w-full p-0 m-0 list-none">
        <li class="w-full">
            <a class="flex flex-col items-center text-inherit" href="{{route('dashboard.user.settings.index')}}">
                <svg class="h-8" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0"></path>
        ...
        <li class="w-full">
            <button class="flex flex-col items-center w-full h-auto p-0 collapsed bg-none bg-transparent rounded-full text-inherit border-none group outline-none translate-x-0 transform-gpu" type="button" data-bs-toggle="collapse" data-bs-target="#navbar-templates" aria-controls="navbar-templates" aria-expanded="false" aria-label="Toggle templates">
                <span style="background: linear-gradient(to right, #8d65e9, #5391e4, #6bcd94);" class="inline-flex items-center justify-center w-[30px] h-[30px] mb-1 relative rounded-full overflow-hidden before:w-full before:h-full before:absolute before:top-0 before:left-0 before:rounded-full before:bg-gradient-to-r before:from-[#8d65e9] before:via-[#5391e4] before:to-[#6bcd94] before:animate-spin-grow">
                    <svg class="relative rotate-[135deg] group-[.collapsed]:rotate-0 transition-transform duration-300" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" stroke-width="1.5" stroke="#fff" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
<div class="collapse fixed right-0 bottom-16 z-20 w-full max-h-[calc(85vh-4rem)] overflow-y-auto rounded-t-2xl shadow-[-5px_-10px_30px_rgba(0,0,0,0.07)] group bg-[#fff] overscroll-contain lg:!hidden dark:bg-zinc-800" id="navbar-templates">
    <ul class="p-0 m-0 h-full relative list-none text-[13px] text-heading font-medium">
        ...
        <li class="relative group">
            <a class="flex items-center gap-2 p-3 py-2 border-solid border-b border-r-0 border-t-0 border-l-0 border-[--tblr-border-color] text-inherit" @if($aiWriter->type == 'text')
...
<div class="fixed bottom-20 end-12 z-50 hidden group lg:block">
    <button class="bg-transparent border-none !outline-none rounded-full w-14 h-14 p-0 overflow-hidden !shadow-lg collapsed translate-x-0 transform-gpu peer" type="button">
        <span class="inline-flex items-center justify-center w-full h-full mb-1 relative rounded-full overflow-hidden before:w-full before:h-full before:absolute before:top-0 before:left-0 before:rounded-full spinner_button before:animate-spin-grow">
            <svg class="relative group-hover:rotate-[135deg] transition-transform duration-300" xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 24 24" stroke-width="3" stroke="#fff" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path d="M12 5l0 14"></path>
                <path d="M5 12l14 0"></path>
            </svg>
        </span>
